...
var Fn = (jsFunc, layout = null) => {
  ...
  fn.once = (namespace3 = null) => {
    shaderNode.once = true;
    shaderNode.namespace = namespace3;
    return fn;
  };
  if (layout !== null) {
...
var normalize = nodeProxy(MathNode, MathNode.NORMALIZE).setParameterLength(1);
...
var min$1 = nodeProxy(MathNode, MathNode.MIN).setParameterLength(2, Infinity);
var max$1 = nodeProxy(MathNode, MathNode.MAX).setParameterLength(2, Infinity);
...
var smoothstep = nodeProxy(MathNode, MathNode.SMOOTHSTEP).setParameterLength(3);
...
var namespace = (node, namespace3) => node.context({ namespace: namespace3 });
...
var cameraIndex = uniform(0, "uint").label("u_cameraIndex").setGroup(sharedUniformGroup("cameraIndex")).toVarying("v_cameraIndex");
...
var cameraProjectionMatrix = Fn(({ camera }) => {
  ...
  if (camera.isArrayCamera && camera.cameras.length > 0) {
    ...
    cameraProjectionMatrix3 = uniform("mat4").label("cameraProjectionMatrix").setGroup(renderGroup).onRenderUpdate(({ camera: camera2 }) => camera2.projectionMatrix);
  }
...
var cameraProjectionMatrixInverse = Fn(({ camera }) => {
  ...
  if (camera.isArrayCamera && camera.cameras.length > 0) {
    ...
    cameraProjectionMatrixInverse3 = uniform("mat4").label("cameraProjectionMatrixInverse").setGroup(renderGroup).onRenderUpdate(({ camera: camera2 }) => camera2.projectionMatrixInverse);
  }
...
var cameraViewMatrix = Fn(({ camera }) => {
  ...
  if (camera.isArrayCamera && camera.cameras.length > 0) {
    ...
    cameraViewMatrix3 = uniform("mat4").label("cameraViewMatrix").setGroup(renderGroup).onRenderUpdate(({ camera: camera2 }) => camera2.matrixWorldInverse);
  }
...
var positionWorldDirection = Fn((builder) => {
...
var getBitangent = Fn(([crossNormalTangent, varyingName], builder) => {
...
var transformedBitangentView = getBitangent(transformedNormalView.cross(transformedTangentView), "v_transformedBitangentView").normalize().toVar("transformedBitangentView");
...
var equirectUV = nodeProxy(EquirectUVNode).setParameterLength(0, 1);
...
var triplanarTextures = nodeProxy(TriplanarTexturesNode).setParameterLength(1, 6);
...
var normal = Fn(({ texture: texture3, uv: uv3 }) => {
  ...
  If(uv3.x.lessThan(epsilon), () => {
    ...
    const step3 = 0.01;
    const x = texture3.sample(uv3.add(vec3(-0.01, 0, 0))).r.sub(texture3.sample(uv3.add(vec3(step3, 0, 0))).r);
    const y = texture3.sample(uv3.add(vec3(0, -0.01, 0))).r.sub(texture3.sample(uv3.add(vec3(0, step3, 0))).r);
    const z = texture3.sample(uv3.add(vec3(0, 0, -0.01))).r.sub(texture3.sample(uv3.add(vec3(0, 0, step3))).r);
...
var shapeCircle = Fn(([coord = uv()], { renderer, material }) => {
...
var _drawingBufferSize$1 = new Vector2();
